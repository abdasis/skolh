            ...
            {visible.map((item) => {
                ...
                return (
                    ...
                    >
                        <Link
                            href={item.href}
                            className="flex items-center justify-between px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-[#006BFF] dark:text-gray-300 dark:hover:bg-gray-800 dark:hover:text-[#3B8BFF]"
                        >
                            {item.label}
                            {hasChildren && level < 3 && (
                                <svg
...
const DesktopNavItem = ({ item }: { item: SiteNavItem }) => {
    ...
    if (!hasChildren) {
        return (
            <Link
                href={item.href}
                className="rounded-lg px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 hover:text-[#006BFF] dark:text-gray-300 dark:hover:bg-gray-800 dark:hover:text-[#3B8BFF]"
            >
                {item.label}
            </Link>
        );
    ...
    return (
        ...
        >
            <Link
                href={item.href}
                className="flex items-center gap-1 rounded-lg px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 hover:text-[#006BFF] dark:text-gray-300 dark:hover:bg-gray-800 dark:hover:text-[#3B8BFF]"
            >
                {item.label}
                <svg
    ...
    return (
        <div>
            <div className="flex items-center">
                <Link
                    href={item.href}
                    className="flex-1 rounded-lg px-3 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 hover:text-[#006BFF] dark:text-gray-300 dark:hover:bg-gray-800"
...
const GuestHeader = ({ canRegister = true }: { canRegister?: boolean }) => {
    ...
    return (
        <header className="fixed top-0 right-0 left-0 z-50">
            {/* Top Bar */}
            <div className="bg-[#006BFF] dark:bg-[#0052CC]">
                ...
                <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
                    ...
                    <Link href={home().url} className="flex items-center gap-3">
                        {siteConfig?.identity.logo_url ? (
                            ...
                            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#006BFF] text-sm font-bold text-white shadow-sm">
                    ...
                    <div className="hidden items-center gap-3 md:flex">
                        {auth.user ? (
                            <Link
                                href={dashboard()}
                                className="rounded-lg bg-[#006BFF] px-6 py-2.5 text-sm font-bold tracking-wide text-white uppercase shadow-sm transition hover:bg-[#0052CC]"
                            >
                                Dashboard
                            </Link>
                        ) : (
                            <>
                                {canRegister && (
                                    <Link
                                        href={register()}
                                        className="rounded-lg bg-[#FFF100] px-6 py-2.5 text-sm font-bold tracking-wide text-gray-900 uppercase shadow-sm transition hover:bg-[#E6D900]"
                                    >
                                        Daftar Sekarang
                                    </Link>
                                )}
                        ...
                        <div className="flex flex-col gap-0.5">
                            ...
                            <div className="mt-3 flex flex-col gap-2 border-t border-gray-100 pt-4 dark:border-gray-800">
                                {auth.user ? (
                                    <Link
                                        href={dashboard()}
                                        className="rounded-lg bg-[#006BFF] py-3 text-center text-sm font-bold tracking-wide text-white uppercase"
                                    >
                                        Dashboard
                                    </Link>
                                ) : (
                                    <>
                                        {canRegister && (
                                            <Link
                                                href={register()}
                                                className="rounded-lg bg-[#FFF100] py-3 text-center text-sm font-bold tracking-wide text-gray-900 uppercase"
                                            >
                                                Daftar Sekarang
                                            </Link>
                                        )}
