...
const DesktopNavItem = ({ item, scrolled }: { item: SiteNavItem; scrolled: boolean }) => {
    ...
    const linkClass = scrolled
        ? 'rounded-lg px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 hover:text-emerald-600 dark:text-gray-300 dark:hover:bg-gray-800 dark:hover:text-emerald-400'
        : 'rounded-lg px-4 py-2 text-sm font-semibold text-white/90 transition hover:bg-white/10 hover:text-white';
...
const HeaderStyleThree = ({ canRegister = true }: { canRegister?: boolean }) => {
    ...
    const headerClass = scrolled
        ? 'border-b border-gray-100 bg-white/95 shadow-sm backdrop-blur-lg dark:border-gray-800 dark:bg-gray-950/95'
        : 'border-b border-white/10 bg-transparent';

    const logoTextClass = scrolled ? 'text-gray-900 dark:text-white' : 'text-white';
    const logoSubClass = scrolled ? 'text-gray-500 dark:text-gray-400' : 'text-white/80';

    return (
        <header className={`fixed top-0 right-0 left-0 z-50 transition-all duration-300 ${headerClass}`}>
            <nav>
                <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
                    {/* Logo */}
                    <Link href={home().url} className="flex items-center gap-3">
                        {siteConfig?.identity.logo_url ? (
                            <img src={siteConfig.identity.logo_url} alt={siteConfig.identity.name} className="h-11 w-11 rounded-xl object-contain" />
                        ) : (
                            <div className={`flex h-11 w-11 items-center justify-center rounded-xl text-sm font-bold text-white shadow-sm ${scrolled ? 'bg-emerald-600' : 'bg-white/20'}`}>
                                {siteConfig?.identity.name?.slice(0, 2).toUpperCase() ?? 'SA'}
                            </div>
                        )}
                        <div>
                            <h1 className={`text-base leading-tight font-extrabold tracking-tight transition-colors ${logoTextClass}`}>
                                {siteConfig?.identity.name ?? ''}
                            </h1>
                            <p className={`text-[10px] font-medium transition-colors ${logoSubClass}`}>{siteConfig?.identity.tagline ?? ''}</p>
                        </div>
                    </Link>
                    ...
                    <div className="hidden items-center gap-1 md:flex">
                        {navItems.map((item) => (
                            <DesktopNavItem key={item.id} item={item} scrolled={scrolled} />
                        ))}
                    </div>
                    ...
                    <div className="hidden items-center gap-3 md:flex">
                        {auth.user ? (
                            <Link href={dashboard()} className="rounded-lg bg-emerald-600 px-6 py-2.5 text-sm font-bold tracking-wide text-white uppercase shadow-sm transition hover:bg-emerald-700">
                                Dashboard
                            </Link>
                        ) : (
                            <>
                                {canRegister && (
                                    <Link href={register()} className="rounded-lg bg-orange-500 px-6 py-2.5 text-sm font-bold tracking-wide text-white uppercase shadow-sm transition hover:bg-orange-600">
                                        Daftar Sekarang
                                    </Link>
                                )}
                            </>
                    ...
                    <button
                        type="button"
                        className={`rounded-lg p-2 md:hidden ${scrolled ? 'text-gray-600 dark:text-gray-300' : 'text-white'}`}
